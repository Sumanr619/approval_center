frappe.pages['approval-follow-up'].on_page_load = function (wrapper) {
	...
	const state = {
		...
		data: { tabs: [], documents: [], summary: { pending: 0, overdue: 0, overdue_3: 0, overdue_7: 0, overdue_14: 0, overdue_30: 0 } },
		sort_by: 'creation',
		sort_order: 'asc',
		tab_scroll_left: 0,
	...
	function apply_filters({ keep_search_focus = false } = {}) {
		state.filters = {
			...
			min_age_days: state.filters.min_age_days || '',
		};
		state.selected_tab = '';
	...
	$container.on('click.approval-follow-up', '[data-af-age]', function () {
		const minimum_age = Number($(this).attr('data-af-age')) || 0;
		state.filters = {
			...state.filters,
			min_age_days: minimum_age && Number(state.filters.min_age_days) !== minimum_age ? minimum_age : '',
		};
		state.selected_tab = '';
		state.tab_scroll_left = 0;
		refresh();
	...
	function render() {
		...
		const selected_age = Number(state.filters.min_age_days) || 0;
		const summary_card = (age, label, value) => `<button class="af-summary-card ${selected_age === age ? 'active' : ''}" data-af-age="${age}" title="${age ? __('Show documents overdue for {0}+ days', [age]) : __('Show all pending workflow documents')}"><span>${label}</span><strong>${value || 0}</strong></button>`;
		const tabs_html = [
			...
			...tabs.map((tab) => `<button class="af-tab ${state.selected_tab === tab.key ? 'active' : ''}" data-af-tab="${escaped(tab.key)}"><span class="af-tab-document">${escaped(tab.doctype)}</span><small class="af-tab-next">${__('Pending')} · ${__('Next role')}: ${escaped(tab.role || __('No role'))}</small><b>${tab.count}</b></button>`),
		].join('');
		...
		$container.html(`
			<style>
				...
				.af-hero { padding: 28px 30px; margin-bottom: 22px; color: #fff; border-radius: 18px; background: linear-gradient(120deg, #172b4d 0%, #1f4b84 60%, #1b6b8e 100%); box-shadow: 0 12px 30px rgba(31,75,132,.18); }.af-hero-top { display:flex; align-items:start; justify-content:space-between; gap:16px; }.af-kicker { margin:0 0 6px; color:#a9d8ff; font-size:12px; font-weight:700; letter-spacing:1.1px; text-transform:uppercase; }.af-hero h2 { margin:0; color:#fff; font-size:27px; font-weight:700; }.af-hero p { margin:8px 0 0; color:#d9e9f8; }.af-refresh { padding:8px 14px; color:#fff; border:1px solid rgba(255,255,255,.28); border-radius:8px; background:rgba(255,255,255,.14); }.af-refresh:hover { color:#fff; background:rgba(255,255,255,.24); }.af-summary { display:flex; gap:12px; overflow-x:auto; padding:2px 0; margin-top:24px; }.af-summary-card { flex:0 0 170px; padding:13px 15px; color:#fff; border:1px solid rgba(255,255,255,.2); border-radius:12px; background:rgba(255,255,255,.12); text-align:left; transition:background .15s ease,transform .15s ease; }.af-summary-card:hover { background:rgba(255,255,255,.22); transform:translateY(-1px); }.af-summary-card.active { border-color:#fff; background:rgba(133,205,255,.34); box-shadow:0 0 0 2px rgba(255,255,255,.23); }.af-summary-card span { display:block; color:#dceeff; font-size:12px; font-weight:600; }.af-summary-card strong { display:block; margin-top:5px; font-size:28px; line-height:1; }
				...
				.af-tabs { display:flex; gap:10px; overflow-x:auto; padding:2px 0 14px; margin-bottom:6px; }.af-tab { display:grid; grid-template-columns:minmax(0,1fr) auto; grid-template-rows:auto auto; align-items:start; column-gap:12px; row-gap:3px; flex:0 0 min(420px,calc(100vw - 58px)); padding:11px 12px 11px 14px; color:#45627f; border:1px solid #dfe6ee; border-radius:13px; background:#fff; font-size:12px; text-align:left; }.af-tab:hover { transform:translateY(-1px); }.af-tab-document { grid-column:1; overflow:visible; color:#294d73; font-size:13px; font-weight:700; line-height:1.3; white-space:normal; }.af-tab-next { grid-column:1; overflow:visible; max-width:none; color:#718096; font-size:11px; line-height:1.35; white-space:normal; }.af-tab b { grid-column:2; grid-row:1 / span 2; align-self:center; min-width:28px; padding:3px 7px; margin:0; color:#334e68; border-radius:12px; background:#eef2f7; font-weight:800; text-align:center; }.af-tab.active { color:#fff; border-color:#0d6db8; background:#1877c9; box-shadow:0 0 0 2px rgba(13,109,184,.2); font-weight:700; }.af-tab.active .af-tab-document { color:#fff; }.af-tab.active .af-tab-next { color:#d9edff; }.af-tab.active b { color:#0d5fa4; background:#fff; }
			...
			<section class="approval-follow-up">
				<div class="af-hero"><div class="af-hero-top"><div><div class="af-kicker">${__('Workflow workspace')}</div><h2>${__('Approval Follow-up')}</h2><p>${__('Read-only visibility of documents awaiting their next workflow transition.')}</p></div><button class="af-refresh" data-af-refresh>↻ ${__('Refresh')}</button></div><div class="af-summary">${summary_card(0, __('Pending workflow documents'), summary.pending)}${summary_card(3, __('Overdue for 3+ days'), summary.overdue_3)}${summary_card(7, __('Overdue for 7+ days'), summary.overdue_7)}${summary_card(14, __('Overdue for 14+ days'), summary.overdue_14)}${summary_card(30, __('Overdue for 1+ month'), summary.overdue_30)}</div></div>
