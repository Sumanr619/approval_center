frappe.pages['approval-follow-up'].on_page_load = function (wrapper) {
	...
	function render() {
		...
		const tabs_html = [
			...
			...tabs.map((tab) => `<button class="af-tab ${state.selected_tab === tab.key ? 'active' : ''}" data-af-tab="${escaped(tab.key)}"><span>${escaped(tab.doctype)}</span><small>${__('Pending')} · ${__('Next role')}: ${escaped(tab.role || __('No role'))}</small><b>${tab.count}</b></button>`),
		].join('');
