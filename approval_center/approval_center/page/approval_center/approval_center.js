frappe.pages['approval-center'].on_page_load = function (wrapper) {
	...
	function apply_toolbar_filters({ keep_search_focus = false } = {}) {
		state.filters = {
	...
	$container.on('input.approval-center', '[name="search"]', function () {
		state.search_cursor = this.selectionStart || 0;
		clearTimeout(search_debounce);
		search_debounce = setTimeout(() => {
			apply_toolbar_filters({ keep_search_focus: true });
		}, 300);
	});
	...
	function render() {
		...
		const cards = documents.length ? documents.map((doc) => {
			...
			const metric_section = stock_metrics ? `<div class="ac-stock-metrics">${stock_metrics}</div>` : '';
			const card_dates = (doc.card_dates || []).map((detail) => {
				const formatted_date = frappe.datetime.str_to_user(detail.value) || detail.value;
				return `<div class="ac-card-date"><small>${escaped(detail.label)}</small><strong>${escaped(String(formatted_date).split(' ')[0])}</strong></div>`;
			}).join('');
			...
			const detail_section = card_details
				? `<div class="ac-divider"></div><div class="ac-details ${doc.doctype === 'Stock Entry' ? 'ac-stock-details' : ''}">${card_details}</div>`
				: '';
			return `<article class="ac-card">
				...
				<div class="ac-card-body">
					...
					<div class="ac-card-main"><div class="ac-card-title"><h3>${escaped(doc.title || doc.name)}</h3>${document_id}</div>${metric_section}</div>
					${date_section}
					${detail_section}
				</div>
		...
		$container.html(`
			<style>
				.approval-center { box-sizing: border-box; width: 100%; max-width: 1440px; margin: 0 auto; padding: 0 22px 36px; color: #172b4d; }
				...
				.ac-toolbar { display: grid; grid-template-columns: repeat(4, minmax(150px, 1fr)); gap: 10px; padding: 16px; margin-bottom: 18px; background: #fff; border: 1px solid #e5eaf0; border-radius: 14px; box-shadow: 0 4px 14px rgba(19,45,83,.05); }.ac-toolbar .form-control { height: 38px; border-radius: 8px; }
				...
				@media (max-width: 900px) { .ac-toolbar { grid-template-columns: repeat(2, minmax(0, 1fr)); } } @media (max-width: 700px) { .approval-center { padding: 0 12px 28px; }.ac-hero { padding: 22px 18px; }.ac-hero-top { align-items: flex-start; }.ac-hero h2 { font-size: 23px; }.ac-toolbar { grid-template-columns: 1fr; }.ac-queue-heading { align-items: stretch; flex-direction: column; }.ac-sort { width: 100%; }.ac-grid { grid-template-columns: 1fr; } }
			</style>
		...
		if (state.restore_search_focus) {
			...
			if (search_input) {
				requestAnimationFrame(() => {
					search_input.focus();
					const cursor = Math.min(state.search_cursor, search_input.value.length);
